...
plotting_networks = function(network_to_plot, spic_network, category, ps_object, tax_rank, plot_file_name, 
    ...
    if(labels == TRUE){
        v_label =V(igraph_filt)$species_names
    }
    else{
        v_label = NA
    }
    vertex_size = log10(apply(otu_table(ps_object), 1, median, na.rm = TRUE)+1)^2.2
    print(vertex_size)
        
    #print(rownames(otu_table(ps_object))[which(vertex_size == min(vertex_size))])
    print(min(vertex_size))
    #print(rownames(otu_table(ps_object))[which(vertex_size == max(vertex_size))])
    print(max(vertex_size))
    
    png(plot_file_name, height = hei, width = wid, units = "in", res = 600)
    plot(igraph_filt,  vertex.size = vertex_size, vertex.label = v_label, main = category,
            edge.width = scales::rescale(E(igraph_filt)$abs_weight, to = c(1, 3)), 
            edge.color = ifelse(E(igraph_filt)$sign == "positive", "black", "red"),
            vertex.color = V(igraph_filt)$color, xlim = c(-1.5, 1.5),
            layout = layout_fr)
...
otu_table_ps$rowmedian = log10(apply(otu_table_ps, 1, median, na.rm = TRUE))*10
#View(otu_table_ps)
print(max(otu_table_ps$rowmedian))
