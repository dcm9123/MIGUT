...
I have been working with ANCOM-BC2 to determine which microbial taxa are differentially abundant, and specifically which taxa are enriched or depleted in Parkinson's disease compared with healthy controls. Microbiome data are compositional because the observed read counts depend on an unknown sampling fraction. ANCOM-BC2 uses the assumption that the microbial community is sparse—that many taxa are absent from most samples—to estimate and correct for sampling-fraction bias, allowing the underlying absolute abundances to be compared. This is preferable to relying only on total-sum scaling (TSS), which converts counts to relative abundances but can introduce bias because an apparent change in one taxon may be caused by changes in the abundance of other taxa.

ANCOM-BC2 also performs a sensitivity analysis to assess whether the results depend on the pseudocount added to zero observations. Taxa whose conclusions change substantially under different pseudocount choices can be flagged as sensitive, reducing the risk of interpreting an artifact of zero handling as a biological result. In a multivariable model, the association for each covariate is tested individually while adjusting for the other covariates included in the model. For example, the Parkinson's disease effect can be estimated while controlling for age and sex, and the age and sex effects are likewise evaluated while controlling for the remaining variables.
